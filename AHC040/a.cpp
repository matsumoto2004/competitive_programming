...
int main() {
    ...
    for(ll border=1e3;border<1e6;border+=1e3){
    ...
    rep(i,n){
        ll row=idsrow[i];
        cout << i << " " << vr[i] << " " << "L " << (row==0? -1:rowid[row-1]) << endl;
        if(vh[rowid[row]]<vh[i]){
            rowid[row]=i;
        }
    }
