...
void solve(){
    ...
    for(auto tox:graph[x]){
        if(tox==y) continue;
        if(id[tox]==id[y]){
            oky=true;
        }
        else{
            mp[{min(id[x],id[tox]),max(id[x],id[tox])}]--;
            mp[{min(id[y],id[tox]),max(id[y],id[tox])}]++;
        }
    }
    for(auto toy:graph[y]){
        if(toy==x) continue;
        if(id[toy]==id[x]){
            okx=true;
        }
        else{
            mp[{min(id[x],id[toy]),max(id[x],id[toy])}]++;
            mp[{min(id[y],id[toy]),max(id[y],id[toy])}]--;
        }
    }
    for(auto [p,val]:mp){
        if(adj_list[p.first][p.second]!=0&&adj_list[p.first][p.second]+val==0) pls--;
        if(adj_list[p.first][p.second]==0&&adj_list[p.first][p.second]+val!=0) pls++;
    }
    if(pls<0||(!okx)||(!oky)) return;
    else{
        swap(id[x],id[y]);
        for(auto[p,val]:mp){
            adj_list[p.first][p.second]+=val;   
        }
    }
